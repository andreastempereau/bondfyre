...
export const createSwipe = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (existingSwipe) {
      res.status(400).json({ message: 'You have already swiped on this profile' });
      return;
    }
...
export const getPotentialMatches = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const swipedUserIds = await Swipe.find({ user: req.user._id })
      .distinct('swipedUser');
    
    // Get IDs of users that have already swiped on the current user
    const swipedByUserIds = await Swipe.find({ swipedUser: req.user._id })
      .distinct('user');
